  ...
  if (filePaths && filePaths.length > 0) {
    filePaths.forEach(filePath => {
      fs.unlink(filePath, err => {
        if (err) {
          return console.error(`删除临时文件失败： ${err.stack}`);
        }
    ...
    request.post(
      ...
      (err, response, body) => {
        ...
        if (!err && response.statusCode === 200) {
          ...
          console.error(`post:${url} error!${response && response.statusCode}`);
          console.error(`error repsonse body is:${body}`);
          reject(formatRequestError(options, err));
        }
      }
    );
  }).catch(error => {
    console.error(`post:${url} error!${error.stack}`);
    return Promise.reject(error);
  });
});
...
module.exports.remoteGetJSON = options => {
  ...
  return new Promise((resolve, reject) => {
    ...
    console.error(`get:${url} error!${error.stack}`);
    return Promise.reject(error);
  });
};
...
module.exports.fileUploadStream = options => {
  ...
  console.info(`POST请求地址:${url};请求头:${stringifyJSON(requestHeaders)}`);
